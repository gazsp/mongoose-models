    ...
    exports.create = function(name, props) {
        ...
        if (props.instanceMethods) {
            Object.keys(props.instanceMethods).forEach(function(i) {
                props.schema.methods[i] = props.instanceMethods[i];
            });
        }

        // Instance Mixins
        if (props.instanceMixins && Array.isArray(props.instanceMixins)) {
            for (var i = 0; i < props.instanceMixins.length; i++) {
                value = props.instanceMixins[i];

                if(typeof value === 'object') {
                    Object.keys(value).forEach(function(functionName) {
                        props.schema.methods[functionName] = value[functionName];
                    });
                }
            };
        }
        ...
        var excludeProps = [
            'schema', 'collection', 'instanceMethods', 'options', 'columns',
            'funcs', 'registerPlugins', 'staticMixins', 'instanceMixins'
        ];
        ...
        // Static Mixins
        if (props.staticMixins && Array.isArray(props.staticMixins)) {
            for (var i = 0; i < props.staticMixins.length; i++) {
                value = props.staticMixins[i];

                if(typeof value === 'object') {
                    Object.keys(value).forEach(function(functionName) {
                        model[functionName] = value[functionName];
                    });
                }
            };
        }
